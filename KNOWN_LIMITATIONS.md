...
| L28 | En re-análisis con `output_dir` previamente poblado (`analyzer/pipeline.py`, clasificación APPLICATION/THIRD_PARTY_OR_FRAMEWORK/UNKNOWN_COMPANION de Incremento 3), `application_folder_names` puede ser incompleto porque la señal BEFORE/AFTER solo identifica carpetas creadas DURANTE esa ejecución, no las que ya existían de una corrida anterior — confirmado empíricamente (validación E2E, `AFL_DataCenter`: en re-análisis, `application_folder_names` colapsa a vacío) | Activa (riesgo residual documentado, no bloqueante) | Bajo hoy — no produce pérdida de evidencia porque `UNKNOWN_COMPANION` siempre se escanea y únicamente `THIRD_PARTY_OR_FRAMEWORK` puede entrar en `skip_top_level`; se activaría solo si una carpeta propia de una app coincidiera por nombre con `THIRD_PARTY_ASSEMBLY_PATTERN`, no observado en el portafolio actual | P3 (documentar, no ejecutar aún) | Sin diseñar todavía — requiere evidencia de un caso real antes de invertir en una solución (Principio 4, `ARCHITECTURAL_PRINCIPLES.md`) | Sin asignar |
